            ...
            StringRequest currentRequest = new StringRequest(Request.Method.POST, "http://api.openweathermap.org/data/2.5/weather?q=seoul&mode=xml&units=metric&appid=e534f98698c54d562309ec63a5b5dcf5", new Response.Listener<String>(){
                ...
                public void onResponse(String response) {
                    try {
                        ...
                        remoteViews.setTextViewText(R.id.widgetText, temperature);

                        AppWidgetManager manager = AppWidgetManager.getInstance(context);
                        manager.updateAppWidget(new ComponentName(context, WidgetReceiver.class), remoteViews);

                        Element symbolNode = (Element)(doc.getElementsByTagName("weather").item(0));
                        String symbol = symbolNode.getAttribute("icon");
                        ...
                        ImageRequest imageRequest = new ImageRequest(url, new Response.Listener<Bitmap>() {
                            ...
                            public void onResponse(Bitmap response) {
                                Log.d("test", "response Image");
                                remoteViews.setImageViewBitmap(R.id.widgetImage, response);
                                AppWidgetManager manager = AppWidgetManager.getInstance(context);
                                manager.updateAppWidget(new ComponentName(context, WidgetReceiver.class), remoteViews);
                                Log.d("test", "update ok");

                            }
                        }, 0, 0, ImageView.ScaleType.CENTER_CROP, null, new Response.ErrorListener() {
                            @Override
                            public void onErrorResponse(VolleyError error) {
                                Log.d("test_error", error.getMessage());
                            }
                        });
